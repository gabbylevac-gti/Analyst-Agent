...
2. **Store hours** — `executeTransform` auto-resolves `STORE_OPEN_HOUR` / `STORE_CLOSE_HOUR` from the endpoint's linked store location record when `endPointId` is provided. **NEVER call `requestContextCard` for store hours under any circumstances.** If `getTransformPipeline` shows these as `source: "deployment_context"` without resolved values, that is expected — they are resolved at transform execution time by looking up `store_locations.hours` for the endpoint. Simply pass `endPointId` to `executeTransform` and they resolve automatically. The only case where `requestContextCard` is needed for deployment context is CSV upload sessions where no `endPointId` is yet linked.
