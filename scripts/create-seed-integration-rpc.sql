-- =============================================================================
-- Create RPC: seed_integration(p_org_id, p_vendor)
-- =============================================================================
-- Run this ONCE in the Supabase SQL editor to register the function.
-- The app calls it via supabase.rpc('seed_integration', { p_org_id, p_vendor })
-- after a new integration is saved.
--
-- SECURITY DEFINER: runs with owner privileges so it can insert seed records
-- regardless of the calling user's RLS policies. The function validates that
-- p_org_id is non-null before doing any writes.
--
-- To add a new vendor later: add an ELSIF branch in the dispatch block below.
-- =============================================================================

CREATE OR REPLACE FUNCTION seed_integration(p_org_id uuid, p_vendor text)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
BEGIN
  ...
  IF p_org_id IS NULL THEN
    RETURN json_build_object('success', false, 'error', 'p_org_id is required');
  END IF;

  -- ── Dispatch by vendor ──────────────────────────────────────────────────────
  IF p_vendor = 'data-realities-radar' THEN
    PERFORM _seed_dr6000(p_org_id);
  ELSE
    RETURN json_build_object('success', false, 'error', 'Unknown vendor: ' || p_vendor);
  END IF;

  RETURN json_build_object('success', true, 'org_id', p_org_id, 'vendor', p_vendor);
...
-- =============================================================================
-- Internal helper: _seed_dr6000(p_org_id)
-- =============================================================================
-- Not callable externally (convention: underscore prefix).
-- Called only by seed_integration above.
-- =============================================================================

CREATE OR REPLACE FUNCTION _seed_dr6000(p_org_id uuid)
RETURNS void
LANGUAGE plpgsql
AS $$
BEGIN
...
SELECT
  ...
  'DR6000 radar sensor paths report. One row per ~1-second position reading per detected target. Use for spatial analysis, heatmaps, trajectory visualization, and ghost path detection.',
  'approved',
  null
WHERE NOT EXISTS (
...
SELECT
  ...
  '{"source": "DR6000 API v2.0", "report_type": "sessions", "row_unit": "one row per person-visit"}'::jsonb,
  '[
    {"column": "processed_at",            "display_name": "Processed At",          "data_type": "timestamp",   "description": "Pipeline processing timestamp.",                                                                               "units": null,      "notes": "Do NOT use for temporal analysis. Use log_creation_time."},
    {"column": "account_id",              "display_name": "Account ID",            "data_type": "identifier",  "description": "Account ID associated with the sensor.",                                                                      "units": null,      "notes": null},
    {"column": "device_id",               "display_name": "Device ID",             "data_type": "identifier",  "description": "ID of the edge device the sensor is associated with.",                                                        "units": null,      "notes": null},
  ...
  'DR6000 radar sensor sessions report. One row per person-visit. Use for traffic volume, dwell time, engagement rates, zone analysis, and proximity metrics. Apply ghost filter before computing engagement rates.',
  'approved',
  null
WHERE NOT EXISTS (
  SELECT 1 FROM datasets
...
SELECT p_org_id, content, type::text, confidence, tags, 'approved'
FROM (VALUES
  (
